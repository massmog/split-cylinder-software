...
function [ out ] = Zloss( resonator, coefficients, null_Z, varargin)
a_l = resonator.a_l;
a_u = resonator.a_u;
L_l = resonator.L_l;
L_u = resonator.L_u;
...
E_s = @(r,z) sum(B_m.*V_m.'.*besselj(1,h_m_s.'.*r).*cos(p_m_s.'.*(z))+C_m.*W_m.'.*besselj(1,h_m_s.'.*r).*sin(p_m_s.'.*(z)),1);
...
H_rho_s = @(r,z) sum(p_m_s.'/(1i.*omega.*m_0).*besselj(1,h_m_s.'.*r).*(-B_m.*V_m.'.*sin(p_m_s.'*z)+C_m.*W_m.'.*cos(p_m_s.'*z)),1);
...
if(ANAL_INT)
    ...
    P_f_u=real(sum(sum(pi/(omega*m_0)^2.*((p_m_s.'.*(-B_m.*(V_m.').*(sin(p_m_s*d/2).')+C_m.*(W_m.').*(cos(p_m_s*d/2)).'))*((p_m_s.'.*(-B_m.*(V_m.').*(sin(p_m_s*d/2).')+C_m.*(W_m.').*(cos(p_m_s*d/2).'))))').*C,1),2));
else
    P_f_u = 1/2*2*pi*integral(@(r) r*abs(H_rho_s(r,d/2)).^2,a_u,b,'ArrayValued',true,'RelTol',int_rtol,'AbsTol',int_atol);
end

% Lower sample region flange loss (normalised)
if(ANAL_INT)
    D=zeros(N_s,N_s);
    for k = 1:N_s
        for l = 1:N_s
            if(k==l)
                D(k,l)=b^2/2*besselj(0,h_m_s(k)*b)^2-a_l^2/2*besselj(0,h_m_s(k)*a_l)^2-a_l^2/2*besselj(1,h_m_s(k)*a_l)^2+a_l./h_m_s(k)*besselj(0,h_m_s(k)*a_l)*besselj(1,h_m_s(k)*a_l);
            else
                D(k,l)=-a_l/(h_m_s(k)^2-h_m_s(l)^2)*(h_m_s(l)*besselj(1,h_m_s(k)*a_l)*besselj(0,h_m_s(l)*a_l)-h_m_s(k)*besselj(0,h_m_s(k)*a_l)*besselj(1,h_m_s(l)*a_l));
            end
        end
    end
    P_f_l=real(sum(sum(pi/(omega*m_0)^2.*((p_m_s.'.*(B_m.*(V_m.').*(sin(p_m_s*d/2).')+C_m.*(W_m.').*(cos(p_m_s*d/2)).'))*((p_m_s.'.*(B_m.*(V_m.').*(sin(p_m_s*d/2).')+C_m.*(W_m.').*(cos(p_m_s*d/2).'))))').*D,1),2));
else
    P_f_l = 1/2*2*pi*integral(@(r) r*abs(H_rho_s(r,-d/2)).^2,a_l,b,'ArrayValued',true,'RelTol',int_rtol,'AbsTol',int_atol);
end

% Sample loss
if(ANAL_INT)
    P_s_dash = omega*e_0*e_r*pi*b^2/2.*sum((abs(B_m.*V_m').*besselj(0,h_m_s'.*b)).^2.*(d.*ones(N_s,1)/2+sin(p_m_s.'.*d)./(2*p_m_s.'))+(-(imag(p_m_s')~=0)+(imag(p_m_s')==0)).*(abs(C_m.*W_m').*besselj(0,h_m_s'.*b)).^2.*(d.*ones(N_s,1)/2-sin(p_m_s.'.*d)./(2*p_m_s.')),1);
else
...
switch MODE
    case 1 % Q MODE
        out = (omega*2*(W_s+W_u+W_l))/(R_s*(P_e_l+P_e_u+P_w_l+P_w_u)+R_s_f*(P_f_l+P_f_u)+tand*P_s_dash);
    case 2 % TAND MODE
        out = ((omega*2*(W_s+W_u+W_l))/Q-R_s*(P_e_l+P_e_u+P_w_l+P_w_u)-R_s_f*(P_f_l+P_f_u))/P_s_dash;
    case 3 % CALIBRATE FLANGE LOSS
        out = omega*m_0/2*((P_f_l+P_f_u)/(((omega*2*(W_s+W_u+W_l))/Q)-R_s*(P_e_l+P_e_u+P_w_l+P_w_u)))^2;
    otherwise
